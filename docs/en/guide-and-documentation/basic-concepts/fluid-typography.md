...
If you don't want to manually input fluid values for each class in the Style panel one by one, you can use custom CSS to apply them easily.

To simplify this process, we've prepared a CSS template that allows you to override values of font size variables and text class font sizes effortlessly. Simply copy and paste the values generated by the **Fluid Type Scale Calculator** into the template and place the code inside `css_custom` in the <mark style="color:green;">**Global Code**</mark> section.
